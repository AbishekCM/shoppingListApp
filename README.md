...
The shoppingListApp  is a simple Deno application that starts on port `7777`.
...
Launching the shoppingListApp application starts the Deno application, a
PostgreSQL server, and a database migration process (Flyway).

## Starting and shutting down

The shoppingListApp application is used with Docker Compose.

- To start the shoppingListApp application, open up the terminal in the folder
  that contains the `docker-compose.yml` file and type `docker compose up`.
- To stop the shoppingListApp application, press `ctrl+C` (or similar) in the
...
The application by default watches for changes in the Deno
code and restarts the application whenever needed.

## Database

When the app is up and running, you can access the PostgreSQL database
...
When the  application is started, Flyway is used to run the SQL
...
for future use. The application uses the `app-cache`-folder
...
The application comes also with simple
